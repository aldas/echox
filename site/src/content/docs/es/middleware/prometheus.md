---
title: Prometheus
description: Genera metrics Prometheus para requests HTTP en Echo.
sidebar:
  order: 15
---

El middleware [Echo Prometheus](https://github.com/labstack/echo-prometheus) genera metrics
Prometheus para requests HTTP.

## Uso

Agrega el módulo requerido:

```bash
go get github.com/labstack/echo-prometheus
```

Agrega el middleware Prometheus y una ruta para servir las metrics recopiladas:
...
Servir metrics desde el mismo servidor que las recopila:
...
Servir metrics en un puerto separado:
...
Salida de ejemplo (para el primer ejemplo):
...
### Servir metrics Prometheus personalizadas

Usa metrics personalizadas con el registry por defecto de Prometheus:
...
O crea tu propio registry y registra metrics personalizadas con él:
...
Se puede pasar un skipper para evitar generar metrics en ciertas URLs:
...
Modificar las definiciones de metrics por defecto de `echoprometheus`:
