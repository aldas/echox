---
title: 会话
description: 基于 gorilla/sessions 的 HTTP 会话管理。
sidebar:
  order: 23
---

Session 中间件便于进行基于 [gorilla/sessions](https://github.com/gorilla/sessions) 的
HTTP 会话管理。默认实现提供基于 Cookie 和文件系统的会话 store；你也可以针对不同后端使用
[社区维护的 store](https://github.com/gorilla/sessions#store-implementations)。
...
一个用于创建会话中间件的函数，以及一个从上下文获取会话的工具函数：
...
此示例暴露两个端点：`/create-session` 创建新会话，`/read-session` 在请求包含会话 ID 时
从会话读取值。
