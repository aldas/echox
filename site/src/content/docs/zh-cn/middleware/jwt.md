---
title: JWT
description: 由 echo-jwt module 提供的 JSON Web Token 认证中间件。
sidebar:
  order: 10
---

JWT 中间件提供 JSON Web Token（JWT）认证。它位于独立 module：
[github.com/labstack/echo-jwt](https://github.com/labstack/echo-jwt)。

行为：

- 对于有效 token，它会在上下文中设置用户并调用下一个处理函数。
- 对于无效 token，它会发送 `401 Unauthorized` 响应。
- 对于缺失或无效的 `Authorization` header，它会发送 `400 Bad Request` 响应。
...
完整示例请参见 [JWT cookbook](/zh-cn/cookbook/jwt/)。
